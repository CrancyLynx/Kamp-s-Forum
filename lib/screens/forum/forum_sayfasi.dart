...
class _ForumSayfasiState extends State<ForumSayfasi> {
  ...
  Future<void> _fetchPosts({bool isInitial = false}) async {
    ...
    try {
      if (isInitial) {
        final pinnedQuerySnapshot = await FirebaseFirestore.instance.collection('gonderiler').where('isPinned', isEqualTo: true).get();
        if (mounted) setState(() => _pinnedPosts = pinnedQuerySnapshot.docs);
      }

      Query query = FirebaseFirestore.instance.collection('gonderiler');
      if (_selectedFilter != 'Tümü') query = query.where('kategori', isEqualTo: _selectedFilter);

      switch (_currentSort) {
        case SortType.newestTopics: query = query.orderBy('zaman', descending: true); break;
        case SortType.mostActive: query = query.orderBy('lastCommentTimestamp', descending: true); break;
        case SortType.mostPopular: query = query.orderBy('commentCount', descending: true); break;
      }

      if (_lastDocument != null && !isInitial) query = query.startAfterDocument(_lastDocument!);
      ...
      if (querySnapshot.docs.isNotEmpty) {
        ...
        if (mounted) {
          setState(() {
            final newPosts = querySnapshot.docs.where((doc) => !_pinnedPosts.any((pinned) => pinned.id == doc.id)).toList();
            _posts.addAll(newPosts);
          });
        }
      }
      if (querySnapshot.docs.length < 15) if (mounted) setState(() => _hasMore = false);
  ...
  void _showCreateOptions(BuildContext context) {
    if (widget.isGuest) { _showLoginRequiredDialog(); return; }
    showModalBottomSheet(context: context, shape: const RoundedRectangleBorder(borderRadius: BorderRadius.vertical(top: Radius.circular(20))), builder: (context) {
        return SafeArea(child: Padding(padding: const EdgeInsets.symmetric(vertical: 20), child: Wrap(children: [
                ListTile(leading: const Icon(Icons.edit_note, color: AppColors.primary), title: const Text("Forum Konusu / İtiraf"), onTap: () { Navigator.pop(context); Navigator.push(context, MaterialPageRoute(builder: (context) => GonderiEklemeEkrani(userName: widget.userName))).then((_) => _resetAndFetch()); }),
                ListTile(leading: const Icon(Icons.poll, color: Colors.purple), title: const Text("Anket Oluştur"), onTap: () { Navigator.pop(context); Navigator.push(context, MaterialPageRoute(builder: (context) => AnketEklemeEkrani(userName: widget.userName))).then((_) => _resetAndFetch()); }),
            ])));
      });
  }
